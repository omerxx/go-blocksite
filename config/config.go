...
func setConfigDefaults() (config Config) {
	config = Config{
		...
		hostsfile:  "/etc/hosts",
		state:      "$HOME/.config/go-blocksite/state.json",
	}
	return config
}
...
func ReadConfig() (config Config) {
	...
	viper.AddConfigPath("$HOME/.config/go-blocksite/")
	// viper.AddConfigPath(".")
...
func RunPreflightChecks(config Config) {
	if len(config.blockList) == 0 {
		log.Println("Nothing in Blocklist, check config file")
	}
	if !canWriteToHosts(config) {
		fmt.Println("Can't write to hostsfile, try using sudo. Aborting.")
		os.Exit(0)
	}
}
